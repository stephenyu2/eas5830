...
contract AMM is AccessControl{
    ...
    */
    function tradeTokens( address sellToken, uint256 sellAmount ) public {
        ...
        //YOUR CODE HERE 
        uint256 eff = sellAmount * (10000 - feebps) / 10000;
        qtyA = ERC20(tokenA).balanceOf(address(this));
        qtyB = ERC20(tokenB).balanceOf(address(this));

        if (sellToken == tokenA) {
            uint256 out = qtyB - (invariant / (qtyA + eff));
            swapAmt = out;
            ERC20(tokenA).transferFrom(msg.sender, address(this), sellAmount);
            ERC20(tokenB).transfer(msg.sender, out);
            emit Swap(tokenA, tokenB, sellAmount, out);
        } else {
            uint256 out = qtyA - (invariant / (qtyB + eff));
            swapAmt = out;
            ERC20(tokenB).transferFrom(msg.sender, address(this), sellAmount);
            ERC20(tokenA).transfer(msg.sender, out);
            emit Swap(tokenB, tokenA, sellAmount, out);
        }
